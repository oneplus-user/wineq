...
def index():
    if request.method == "POST":
        ...
        try:
            if request.form:
                response = prediction.form_response(dict(request.form))
                #return jsonify(response)                               #for POSTMAN
                return render_template("index.html", response=response) #for Browser-based app
            elif request.json:
                response = prediction.api_response(request.json)        
                return jsonify(response)                                #only possible for POSTMAN (error or no error: always return valid response)

        except prediction.NotInRange as e:                              # only valid of form request(col error can't happen in form request)
            error = {"error": e}
            # return jsonify(error)                                     #for POSTMAN
            return render_template("404.html", error=error)             #for Browser-based app
